F44RedCoin Core version 0.18.0 is now available from:

  <https://f44redcoincore.org/bin/f44redcoin-core-0.18.0/>
...
`/Applications/F44RedCoin-Qt` (on Mac) or `f44redcoind`/`f44redcoin-qt` (on
Linux).
...
recommended to use F44RedCoin Core on unsupported systems.

F44RedCoin Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, F44RedCoin Core does not yet change appearance when
...
`contrib/init/f44redcoind.service`, it has been changed to use
`/var/lib/f44redcoind` as the data directory instead of
`~f44redcoin/.f44redcoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/f44redcoind` will
exist has enough space (check using `df -h /var/lib/f44redcoind`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/f44redcoin/f44redcoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://f44redcoincore.org/en/contact/) the F44RedCoin Core
...
The systemd init file (`contrib/init/f44redcoind.service`) has been changed
to use `/var/lib/f44redcoind` as the data directory instead of
`~f44redcoin/.f44redcoin`. This change makes F44RedCoin Core more consistent with
...
`datadir` option in `/etc/f44redcoin/f44redcoin.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/f44redcoin/f44redcoin.conf`.


Documentation
-------------

- A new short [document](https://github.com/f44redcoin/f44redcoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/f44redcoin/f44redcoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/f44redcoin/f44redcoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/f44redcoin/f44redcoin/blob/master/doc/f44redcoin-conf.md)
  about the `f44redcoin.conf` file describes how to use it to configure
  F44RedCoin Core.

- A new document introduces F44RedCoin Core's BIP174 [Partially-Signed
  F44RedCoin Transactions
  (PSBT)](https://github.com/f44redcoin/f44redcoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/f44redcoin/f44redcoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent F44RedCoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  F44RedCoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/f44redcoin/f44redcoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/f44redcoin/f44redcoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when F44RedCoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of F44RedCoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/f44redcoin/f44redcoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/f44redcoin/f44redcoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart f44redcoind with the `-deprecatedrpc=generate`
...
- A new `f44redcoin-wallet` tool is now distributed alongside F44RedCoin
...
This section describes planned changes to F44RedCoin Core that may affect
other F44RedCoin software and services.

- Since version 0.16.0, F44RedCoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with F44RedCoin Core 0.20 (expected about
  a year after 0.18), F44RedCoin Core will default to native segwit
  ...
  to bech32 addresses, and if the F44RedCoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in F44RedCoin Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any F44RedCoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, F44RedCoin Core
...
- A sub-project of F44RedCoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with F44RedCoin Core.  See their [project
  page](https://github.com/f44redcoin-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to F44RedCoin Core's own implementation, although entropy
  gathered by F44RedCoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves F44RedCoin Core a
...
- On macOS, F44RedCoin Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `f44redcoin_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libf44redcoin_server.a` (jnewbery)
...
- #13926 [Tools] f44redcoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like f44redcoind (MarcoFalke)
...
- #14521 qt, docs: Fix `f44redcoin-qt -version` output formatting (hebasto)
...
- #11625 Add F44RedCoinApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libf44redcoin_server` (laanwj)
...
- #15154 Configure: f44redcoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/f44redcoin-wallet (Sjors)
...
- #11526 Visual Studio build configuration for F44RedCoin Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libf44redcoin_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in f44redcoin-util-test.py (isghe)
...
- #12775 Integration of property based testing into F44RedCoin Core (Christewart)
...
- #12246 Bugfix: Only run f44redcoin-tx tests when f44redcoin-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_f44redcoin` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_f44redcoin` (MarcoFalke)
...
- #14186 f44redcoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_f44redcoin.conf` message on startup if conf file exists (leishman)
...
- #12255 Update f44redcoin.service to conform to init.md (dongcarl)
...
- #14497 Add doc/f44redcoin-conf.md (hebasto)
...
- #15354 Add missing `f44redcoin-wallet` tool manpages (MarcoFalke)
...
- #15765 correct f44redcoinconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support f44redcoin folder on OSX (JimmyMow)
