F44RedCoin Core version 0.11.0 is now available from:

  <https://f44redcoin.org/bin/f44redcoin-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer F44Coins/kB fee to be
...
For example, add the following to `f44redcoin.conf`:
...
related to the blockchain in the f44redcoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows F44RedCoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that F44RedCoin Core will 
...
For an example script see [f44redcoin-submittx](https://github.com/laanwj/f44redcoin-submittx).
...
- #5334 `cf87536` libf44redcoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new f44redcoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only F44RedCoinAmountField
- #5753 `0f44672` Add f44redcoin logo to about screen
...
- #5809 `46bfbe7` Add f44redcoin-cli man page
...
- #5849 `41259ca` contrib/init/f44redcoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename F44RedCoin-Qt.app to F44RedCoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for f44redcoin-tx)
