F44RedCoin Core version 0.19.0.1 is now available from:

  <https://f44redcoincore.org/bin/f44redcoin-core-0.19.0.1/>
...
F44RedCoin Core is supported and extensively tested on operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not recommended
to use F44RedCoin Core on unsupported systems.
...
to open the F44RedCoin Core .dmg. This is due to new signing requirements
imposed by Apple, which the F44RedCoin Core project does not yet adhere too.
...
- [Reduce memory](https://github.com/f44redcoin/f44redcoin/blob/master/doc/reduce-memory.md)
  suggests configuration tweaks for running F44RedCoin Core on systems with
...
- `-blockfilterindex` enables the creation of BIP158 block filters for
  ...
  currently use about 4 GiB of space.  Note: this version of F44RedCoin
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `f44redcoind -help` output for these two options for
...
- In 0.18.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 0.19.0, this flag is now __disabled__ by default. If you want to compile F44RedCoin Core with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in F44RedCoin Core 0.18.
...
- BIP 61 reject messages were deprecated in v0.18. They are now disabled
  ...
  future version of F44RedCoin Core. (#14054)

- To eliminate well-known denial-of-service vectors in F44RedCoin Core,
  ...
  This prevents F44RedCoin Core from sending the BIP111 NODE_BLOOM service
  ...
  of F44RedCoin Core and nodes that have manually enabled BIP37 support,
...
- By default, F44RedCoin Core will now make two additional outbound connections that are exclusively used for block-relay.  No transactions or addr messages will be processed on these connections. These connections are designed to add little additional memory or bandwidth resource requirements but should make some partitioning attacks more difficult to carry out. (#15759)

Miscellaneous CLI Changes
-------------------------

- The `testnet` field in `f44redcoin-cli -getinfo` has been renamed to
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how F44RedCoin Core determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  F44RedCoin Core would sequentially attempt to download the transaction from each
...
- If a Tor hidden service is being used, F44RedCoin Core will be bound to
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 F44RedCoin address without
...
- When creating a transaction with a fee above `-maxtxfee` (default 0.1
  F44Coins), the RPC commands `walletcreatefundedpsbt` and
  `fundrawtransaction` will now fail instead of rounding down the fee.
  Be aware that the `feeRate` argument is specified in F44Coins per 1,000
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  partial spends" (added in F44RedCoin Core v0.17.0), this can eliminate a
...
- #15649 Add ChaCha20Poly1305@F44RedCoin AEAD (jonasschnelli)
...
- #15639 f44redcoin-wallet tool: Drop libf44redcoin_server.a dependency (ryanofsky)
...
- #16521 Use the default maxfeerate value as F44Coins/kB (Remagpie)
...
- #16366 init: Use InitError for all errors in f44redcoind/qt (MarcoFalke)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "f44redcoin-binaries/version" directory (hebasto)
...
- #16258 [MSVC]: Create the config.ini as part of f44redcoind build (NicolasDorier)
...
- #16441 remove qt libjpeg check from f44redcoin_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_f44redcoin to auto generated project files (sipsorcery)
...
- #15773 Add F44RedCoinTestFramework::sync_* methods (MarcoFalke)
...
- #15352 Reduce noise level in test_f44redcoin output (practicalswift)
...
- #15896 feature_filelock, interface_f44redcoin_cli: Use PACKAGE_NAME in messages rather than hardcoding F44RedCoin Core (luke-jr)
...
- #16277 Suppress output in test_f44redcoin for expected errors (gertjaap)
...
- #16929 follow-up to rpc: default maxfeerate value as F44Coins/kB (jonatack)
...
- #16489 log: harmonize f44redcoind logging (jonatack)
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (f44redcoinVBR)

### Documentation
- #15514 Update Transifex links (fanquake)
- #15513 add "sections" info to example f44redcoin.conf (fanquake)
...
- #15683 Comment for seemingly duplicate LIBF44REDCOIN_SERVER (Bushstar)
...
- #16080 build/doc: update f44redcoin_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in f44redcoin.conf (fanquake)
...
- #16621 add default f44redcoin.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the f44redcoin-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, f44redcoin.conf, and Info.plist.in (ch4ot1c)
...
- #16912 Remove Doxygen intro from src/f44redcoind.cpp (ch4ot1c)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/f44redcoin/f44redcoin/).
