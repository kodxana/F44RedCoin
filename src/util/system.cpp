...
const char * const F44REDCOIN_CONF_FILENAME = "f44redcoin.conf";
 ...
 * https://github.com/f44redcoin/f44redcoin/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //f44redcoin-tx using stdin
        std::string val;
...
#else
    const char* pszModule = "f44redcoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\F44RedCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\F44RedCoin
    // Mac: ~/Library/Application Support/F44RedCoin
    // Unix: ~/.f44redcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "F44RedCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/F44RedCoin";
#else
    // Unix
    return pathRet / ".f44redcoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", F44REDCOIN_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure F44RedCoin Core copyright is not removed by accident
    if (copyright_devs.find("F44RedCoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The F44RedCoin Core developers";
    }
