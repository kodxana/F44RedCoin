This directory contains the F44RedCoinQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
Represents the main window of the F44RedCoin UI.
...
ClientModel is used by the main application `f44redcoingui` and several models like `peertablemodel`.
...
Used to process BIP21 payment URI requests. Also handles URI based application switching (e.g. when following a f44redcoin:... link from a browser).
...
* don't change `local/f44redcoin_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "f44redcoin-qt" under "Run", which is where you can also add command line arguments)
