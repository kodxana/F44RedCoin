...
#include <qt/f44redcoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a F44RedCoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new F44RedCoinAddressEntryValidator(parent));
    widget->setCheckValidator(new F44RedCoinAddressCheckValidator(parent));
}

bool parseF44RedCoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no f44redcoin: URI
    if(!uri.isValid() || uri.scheme() != QString("f44redcoin"))
        return false;
...
bool parseF44RedCoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseF44RedCoinURI(uriInstance, out);
}

QString formatF44RedCoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("f44redcoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;
...
bool openF44RedCoinConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", F44REDCOIN_CONF_FILENAME));
    ...
    /* Open f44redcoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "F44RedCoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "F44RedCoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("F44RedCoin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for F44RedCoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "f44redcoin.desktop";
    return GetAutostartDir() / strprintf("f44redcoin-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a f44redcoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=F44RedCoin\n";
        else
            optionFile << strprintf("Name=F44RedCoin (%s)\n", chain);
