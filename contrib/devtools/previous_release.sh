  ...
  do
    ...
    if [ "$DOWNLOAD_BINARY" -eq "0" ]; then
      ...
      if [ ! -d "$tag" ]; then
        ...
        git clone https://github.com/f44redcoin/f44redcoin "$tag"
        pushd "$tag" || exit 1
        {
          git checkout "$tag"
          ...
          mkdir bin
          mv src/f44redcoind src/f44redcoin-cli src/f44redcoin-tx bin
          if [ "$FUNCTIONAL_TESTS" -eq "0" ]; then
            mv src/qt/f44redcoin-qt bin
          fi
        }
        popd || exit 1
      fi
      ...
      if [ -d "$tag" ]; then
        ...
        if [[ "$tag" =~ v(.*)(rc[0-9]+)$ ]]; then
            BIN_PATH="bin/f44redcoin-core-${BASH_REMATCH[1]}/test.${BASH_REMATCH[2]}"
        else
            BIN_PATH="bin/f44redcoin-core-${tag:1}"
        fi
        URL="https://f44redcoin.org/$BIN_PATH/f44redcoin-${tag:1}-$PLATFORM.tar.gz"
        echo "Fetching: $URL"
        if ! curl -O -f $URL; then
            echo "Download failed."
            exit 1
        fi
        tar -zxf "f44redcoin-${tag:1}-$PLATFORM.tar.gz" -C "$tag" --strip-components=1 "f44redcoin-${tag:1}"
        rm "f44redcoin-${tag:1}-$PLATFORM.tar.gz"
      fi
    fi
  done
}
